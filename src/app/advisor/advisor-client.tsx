...
const SUGGESTIONS = [
  // 具体例句，不是「帮我梳理一个想法」这种元请求：Hermes 会从这句里取出「一句话想法」，
  // 再如实说清入库还缺哪几项。写成泛化的求助句反而什么也提取不到。
  "我想做一款给敏感肌的氨基酸洁面",
...
const PRODUCT_SUGGESTIONS = [
  // 这里只放对话真的能做成的事。原来第一条是「启动一轮完整产品研发」，
  // 但顾问通道没有启动研发的意图，点了只会返回阶段计数 —— 等于承诺了做不到的事。
  // 研发启动的真实控件在产品页，对话里改字段才是这条通道自己的能力。
  "把目标人群改成 25-35 岁通勤女性",
  "总结这个产品当前结论、最大风险和下一步",
  "接下来最应该补什么证据，为什么",
  "挑战我的判断：这个产品假设哪里最脆弱？",
...
export default function AdvisorClient({
  ...
  // Desktop Runtime 与 Kern 顾问团异步任务都通过 Message 协议回写原会话。
  // 轻量轮询让用户停留在对话页时直接看到真实回执，不需要手动刷新。
  React.useEffect(() => {
    if (!convoId) return;
    let disposed = false;
