"use client";
/** 会话与就地卡片：科恩的回答是可读的散文 + 少量结构化卡片，不是仪表盘。 */
...
/** 审批 check-in。科恩只在这种时刻打断人，所以它必须是页面上最重的东西。 */
  ...
  return (
    <div className="m-checkin" data-t={tone}>
      ...
      <div className="m-checkin-body">
        ...
        <p className="m-hint">{d.raisedBy} 提出{d.gate ? ` · ${d.gate}` : ""}。确认后科恩会自己继续，不用你盯着。</p>
      </div>
    </div>
  );
}
...
function Block({ b, employees, onOpenSource }: { b: MessageBlock; employees: Employee[]; onOpenSource: (r: EvidenceRef) => void }) {
  switch (b.kind) {
      ...
      return (
        <Card>
          ...
          <div className="m-card-body">
            ...
            <p className="m-hint">科恩不直接改业务数据，改动一律先给你看。</p>
          </div>
        </Card>
      );
    case "verdict": {
    ...
    case "unknown":
      return (
        <Card>
          <CardHead icon={<I.spark />} title="缺口：科恩不替你猜" />
          <div className="m-card-body">
