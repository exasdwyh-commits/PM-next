...
test("VNext Judgment：provider key 与 engine kind 解耦，MODEL provider 不可绕过 RULES_ONLY", async () => {
  const specs = createDefaultDecisionSpecs();
  const kernel = new DecisionIntelligenceKernel(specs);
  ...
  fake.register("workforce.route_agent", "v2", () => ({
    value: "research_agent",
    confidence: 0.98,
    reasonCodes: ["MODEL_ROUTE"],
    calibrated: true,
    calibrationProfile: "route-agent/cal-v1",
    benchmarkProfile: "route-agent/bench-v1",
  }));
  kernel.registerProvider(fake);
  ...
  const result = await kernel.decideWithProvider(
    {
      ...
      contextRefs: ["work-item:vnext-provider"],
    },
    "laya-shadow"
  );
  ...
  assert.equal(result.engineResult.providerVersion, "laya-shadow/test-v1");
  assert.equal(result.engineResult.value, "research_agent");
  assert.equal(result.policy.action, "ESCALATE_AGENT");
});

test("VNext Judgment：abstain 即使高置信且有 benchmark 也永不 AUTO", async () => {
  const specs = new DecisionSpecRegistry();
  specs.register({
    key: "test.abstention",
    version: "v1",
  ...
  const fake = new FakeJudgmentProvider("candidate-model", "MODEL", "fake/v1");
  fake.register("test.abstention", "v1", () => ({
    value: true,
    confidence: 0.999,
    calibrated: true,
    calibrationProfile: "cal/v1",
  ...
  assert.equal(result.engineResult.abstained, true);
  assert.equal(result.policy.action, "ESCALATE_AGENT");
  assert.match(result.policy.reasons.join(" "), /abstained/i);
});

test("VNext Judgment：默认 Intelligence Level 规则不触发 MoA，核心分析才进入 L2", async () => {
  const { kernel } = buildDefaultKernel();

  const routine = await kernel.decide({
    decisionKey: "intelligence.route_level",
    state: {},
    contextRefs: ["task:routine"],
  });
  assert.equal(routine.engineResult.value, "L0");
  assert.equal(routine.policy.action, "AUTO");

  const core = await kernel.decide({
    decisionKey: "intelligence.route_level",
    state: { coreProduct: true },
    contextRefs: ["task:core-product"],
  });
  assert.equal(core.engineResult.value, "L2");
  assert.ok(core.engineResult.reasonCodes.includes("CORE_ANALYSIS_REQUIRED"));
});

test("VNext Judgment：Completion 只做完成度判断，不把缺验证的任务假绿", async () => {
  const { kernel } = buildDefaultKernel();

  const missing = await kernel.decide({
    decisionKey: "completion.status",
    state: {
      requiredChecksPassed: true,
      acceptanceMet: true,
      verificationPending: true,
    },
    contextRefs: ["commit:abc"],
  });
  assert.equal(missing.engineResult.value, "VERIFY_MORE");

  const blocked = await kernel.decide({
    decisionKey: "completion.status",
    state: {
      requiredChecksPassed: true,
      acceptanceMet: true,
    ...
    contextRefs: ["commit:def"],
  });
  assert.equal(blocked.engineResult.value, "INCOMPLETE");

  const complete = await kernel.decide({
    decisionKey: "completion.status",
    state: {
      requiredChecksPassed: true,
      acceptanceMet: true,
    },
    contextRefs: ["commit:ghi"],
  });
  assert.equal(complete.engineResult.value, "COMPLETE");
});

test("VNext Judgment：legacy DecisionEngine 仍可工作并记录 provider provenance", async () => {
  const { kernel } = buildDefaultKernel();
  const result = await kernel.decide({
    decisionKey: "workforce.route_agent",
  ...
  assert.equal(result.engineResult.providerKey, "legacy:rules");
  assert.equal(
    result.engineResult.providerVersion,
    result.engineResult.engineVersion
  );
});


test("VNext Judgment：preferredEngine 不得触发 Spec 未授权 provider", async () => {
  const specs = new DecisionSpecRegistry();
  specs.register({
    key: "test.rules_only",
    version: "v1",
  ...
  rules.register("test.rules_only", "v1", () => ({
    value: true,
    reasonCodes: ["RULES_PATH"],
  }));

  let modelCalled = false;
  const fake = new FakeJudgmentProvider("disallowed-model", "MODEL", "fake/v1");
  fake.register("test.rules_only", "v1", () => {
    modelCalled = true;
    return {
      value: false,
      confidence: 0.99,
