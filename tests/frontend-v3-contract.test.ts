...
test("KERN is the primary operating shell and management stays independent", () => {
  ...
  assert.ok(root.includes('redirect("/muse")'), "root must enter the compatibility /muse shell by default");
  assert.ok(muse.includes('fetch("/api/conversations"'), "KERN shell must create real conversations");
  assert.ok(
    muse.includes('/api/conversations/${conversationId}/messages'),
    "KERN shell must send work through the real assistant conversation API"
  );
  assert.ok(
    muse.includes('/api/proposals/${decision.id}/confirm'),
    "KERN check-ins must use the real proposal approval API"
  );
  assert.ok(shell.includes('label: "返回科恩"'), "management must have an explicit route back to KERN");
...
test("management conversations route back through KERN", () => {
  ...
  assert.ok(home.includes("今天想让科恩做什么？"));
  assert.ok(home.includes("/muse?query="), "management commands must enter the KERN operating shell via the compatibility route");
  assert.ok(home.includes("需要你处理"));
  assert.ok(home.includes("科恩正在工作"));
...
test("product remains the primary business object", () => {
  ...
  assert.ok(product.includes("/muse?product="), "product discussion must open KERN with product context via the compatibility route");
