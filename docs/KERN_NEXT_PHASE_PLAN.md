...
> **优先级：`docs/KERN_WORKING_MEMORY.md` 是唯一可信的进度汇总，冲突时以它为准。** 本文件只写 P1 各项的验收细节。
> **当前阶段是 Display Layer（`docs/KERN_DISPLAY_SPEC.md`）**，下面的 P0-B 起顺延到它之后。
> 继续工作前：先读 Working Memory 与 `docs/KERN_DELIVERY_LEDGER.md`，再核对 **`main` 最新提交 / 开放 PR / CI**。
